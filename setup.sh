...
configure_git() {
    ...
    if [[ "${SCRIPT_DIR}" =~ [Vv][Ii][Tt][Aa] || "${SCRIPT_DIR}" == */DATA*/* ]]; then
        log_info "https://github.com/vita-linux/${project}"
        git -C "${SCRIPT_DIR}" config --local user.name "Vita-Linux"
        git -C "${SCRIPT_DIR}" config --local user.email "vita-arch@proton.me"
        git -C "${SCRIPT_DIR}" remote set-url origin "git@github.com:vita-linux/${project}.git"
        log_success "Git configured — remote set to git@github.com:vita-linux/${project}"
